...
jQuery(document).ready(function($){
	...
	YRC.auth = {
		...
		'baseUrl': function ( rl ){ return '/searchmyvideo/' + rl +'&key=' + this.apikey; },
	...
	YRC.Base.prototype = {		
		...
		'fetch': function(){			
			...
			var url = window.YRC.auth.url( 'searchmyvideo', this.request.page, this.channelOrId(), this.criteria, this.per_page, this.vst ), yc = this;
			url = window.YRC.auth.url( 'Uploads', this.request.page, this.channelOrId(), this.criteria, this.per_page, this.vst ), yc = this;
			console.log(url);
		...
		'onResponse': function( re ){
			...
			var myprevtoken=String(this.request.times)+'-prev-'+String(this.custom_vids_length)+'-token-'+String(this.per_page);
			this.request.times += this.request.dir;
			var mynexttoken=String(this.request.times)+'-next-'+String(this.custom_vids_length)+'-token-'+String(this.per_page);
				...
				this.request.pagetokens = {'prev': re.myprevtoken, 'next': re.mynexttoken};

                		this.more( re.nextPageToken, Math.min(this.max, re.pageInfo.totalResults) - (this.request.times*this.per_page) );

			}
                       				this.request.pagetokens = {'prev': myprevtoken, 'next': mynexttoken};
console.log(re.nextPageToken, Math.min(this.max, re.pageInfo.totalResults) - (this.request.times*this.per_page));
                		this.more( re.nextPageToken, Math.min(this.max, re.pageInfo.totalResults) - (this.request.times*this.per_page) );
...
//			}*/
	...
	YRC.play = function(yc, a, vid){
		console.log( yc,a,vid );

                          $(myModal).on('shown.bs.modal', function() {
                              $('.modal-backdrop').css('background', 'FFF');
                              $('.modal-backdrop').css('z-index', '10004');
                          ...
                          $(myModal).on('hidden.bs.modal', function() {
                            ...
                            $.ajax({
                              url: "/myvideo?code="+a.attr('href').split('v=')[1],
                              success:function(data){
                                $(masupervideo).attr('src',("//www.youtube.com/embed/"+data+"?enablejsapi=1&amp;rel=0&amp;modestbranding=1&amp;controls=1undefined"));
